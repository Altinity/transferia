...
func TestSnapshot(t *testing.T) {
	...
	source := pgrecipe.RecipeSource(
		pgrecipe.WithInitFiles(yatest.SourcePath("transfer_manager/go/tests/e2e-core/pg2ch/dbt/init_pg.sql")),
		pgrecipe.WithoutPgDump(),
	)
	target := chrecipe.MustTarget(
		chrecipe.WithInitFile(yatest.SourcePath("transfer_manager/go/tests/e2e-core/pg2ch/dbt/init_ch.sql")),
