...
var (
	...
	Source       = *pgrecipe.RecipeSource(pgrecipe.WithInitDir(helpers.RepoPath("tests", "e2e", "pg2ch", "alters", "dump", "pg")), pgrecipe.WithPrefix(""))
	Target       = *chrecipe.MustTarget(chrecipe.WithInitDir(helpers.RepoPath("tests", "e2e", "pg2ch", "alters", "dump", "ch")), chrecipe.WithDatabase(databaseName))
)
